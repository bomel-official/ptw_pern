...
export const paymentService = {
    async createInvoice(
    ...
    ): Promise<{ invoice: Invoice; participant: Participant }> {
        ...
        const { data: yookassaInvoice } = await axios.post(
            "https://api.yookassa.ru/v3/payments", {
                ...
                confirmation: {
                    type: "redirect",
                    return_url: `${ getEnv( process.env.CLIENT_URL ) }/tournament/${ tournament.slug }`,
                },
                ...
                headers: { ...YOOKASSA_HEADERS, "Idempotence-Key": uuidv4() },
                auth: yookassaAuth(),
            } );
    ...
    async getStatus( participantId: number ): Promise<boolean> {
        const participant = await ParticipantRepository.findByPk( participantId );
        if ( !participant ) {
            return false;
        }
