...
export const TournamentPreview = ({item, status, columns, isHideParticipants}: {item: any, status: IResultStatus | undefined, columns: number, isHideParticipants?: boolean}) => {
    ...
    return (
        <li className={`previewVertical c${columns} ${(now.getTime() <= dateEnd.getTime()) ? 'active' : 'finished'}`}>
            <div className="previewVertical__top">
                <img src={getFile(item.previewImg)} alt="Tournament name" className="previewVertical__img"/>
                <span className="previewVertical__status active">{ResultStatuses[(now.getTime() <= dateEnd.getTime() ? 'active' : 'finished')].previewName}</span>
            </div>
            <div className="previewVertical__bottom">
                ...
                {!isHideParticipants && <div className="previewVertical__users">
                    <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path
