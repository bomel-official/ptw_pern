...
export const TournamentsList = ({status, columns, type = 'tournament', userId}: {status?: IResultStatus, columns: number, type?: 'tournament'|'hub', userId?: number}) => {
    ...
    return (
        <ul className={`tournamentsList ${status}`}>
            {data.map((item) => (
                <TournamentPreview item={item} status={status} columns={col} key={item.id} isHideParticipants={!!userId}/>
            ))}
