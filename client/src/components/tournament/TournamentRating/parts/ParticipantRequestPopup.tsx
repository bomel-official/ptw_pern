...
export const ParticipantRequestPopup: FC<{isActive: boolean, onHide: () => void, participants: Array<IParticipant>, changeCertainParticipant: (index: number, value: IParticipant) => void}> = ({isActive, onHide, participants, changeCertainParticipant}) => {
    ...
    for (let i = 0; i < AMOUNT_ROUNDS; i++) {
        ...
        roundsInfo.push(<td key={`round-${i}`} className={(participantRequest?.isRoundsHidden.length && participantRequest?.isRoundsHidden[i]) ? 'transparent' : ''}>
            ...
            <div className="flex mt24" style={{flexDirection: 'column'}}>
                <div className="flex" style={{flexDirection: 'column', alignItems: 'stretch', gap: '8px'}}>
                    <label htmlFor="teamAvatarImage" className="fileInput input-tl corner-margin">
                        <span className="fileInput__text">{participantRequest.approveFilename ? participantRequest.approveFilename : __('Добавить подтверждение')}</span>
                        ...
                        <input
                            ...
                            id="teamAvatarImage"
                            style={{padding: 0}}
                            onChange={(e: ChangeEvent<HTMLInputElement>) => changeParticipantRequestField('approve', e.target['files'] ? e.target['files'][0] : null)}
                        />
                    </label>
