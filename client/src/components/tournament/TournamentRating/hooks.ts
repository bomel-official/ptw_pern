...
export const useParticipantRequest = (participants: Array<IParticipant>, user: IUser | null, token: string | null) => {
    ...
    const changeParticipantRequestField = <TKey extends keyof IParticipantRequestDTO>(key: TKey, value: IParticipantRequestDTO[TKey]) => {
    ...
    const saveHandler = async (): Promise<IParticipantRequest | string> => {
        try {
            const {participantRequest: updatedParticipantRequest} = await request(`/api/tournament/create-participant-request`, 'POST', parseFormData(participantRequest ?? {}), {
