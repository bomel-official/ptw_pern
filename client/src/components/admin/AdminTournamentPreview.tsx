...
export const AdminTournamentPreview = ({item, columns}: {item: any, columns: number}) => {
        ...
        <>
            <li className={`previewVertical c${columns} ${(new Date(Date.now()) <= dateEnd) ? 'active' : 'finished'}`}>
                <div className="previewVertical__top">
                    ...
                    <span className="previewVertical__status active">{(new Date(Date.now()) <= dateEnd) ? ResultStatuses['active'].previewName : ResultStatuses['finished'].previewName}</span>
                </div>
                <div className="previewVertical__bottom">
                    ...
                    <div className="previewVertical__users">
                        ...
                        <span>{item.participantsList.length} / {item.maxUsers} {__('участников')}</span>
                    </div>
                    <div className="previewVertical__buttons pt32">
                        <button className="previewVertical__link" onClick={() => setIsEditorActive(!isEditorActive)}>{__('Редактировать')}</button>
                        <NavLink to={`/tournament/${item.slug}/about`} target="_blank" className="previewVertical__link">{__('Подробнее')}</NavLink>
                    </div>
                </div>
            </li>
            {isEditorActive && <div className="previewVertical__editor">
                <AdminTournamentCreate tournament={item}/>
            </div>}
        </>
    )
}
