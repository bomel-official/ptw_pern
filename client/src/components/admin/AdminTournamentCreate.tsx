...
const AdminTournamentCreate = ( { tournament = newTournamentTemplate }: { tournament?: any } ) => {
    ...
    return (
        <form onSubmit={ createHandler } action={ "null" } method="POST">
            <h2 className="profile__heading mb12">{ tournament.id ?
                __( "Редактировать" ) + ": " + _f( newTournament, "title" ) : __( "Создать турнир" ) }</h2>
                ...
                <div className="build__grid-row">
                    <Select
                        options={ Object.values( Games )
                            .map( ( game: IGameObject ) => ({ value: game.id, text: game.name }) ) }
            ...
            <div className="build__data-block">
                ...
                <div className="build__grid-row">
                    <label htmlFor={ `${ newTournament.id ? newTournament.id : "" }previewImg` }
                           className="input-tl fileInput"
                           style={ { justifyContent: "flex-start", paddingLeft: "8px", paddingRight: "8px" } }>
                        ...
                        <span className="fileInput__text">{ newTournament.previewImg ?
                            newTournament.previewImg.name || newTournament.previewImg : __( "Изображение" ) }</span>
                        <input
                            type="file"
                            ...
                            onChange={ e => changeTournamentField( "previewImg",
                ...
                <div className="build__grid-row">
                    <label htmlFor="descAdditional_RU" className="input"
                    ...
                    <label htmlFor="descAdditional_EU" className="input-br"
                           style={ { height: "auto", minHeight: "64px" } }>
                        <textarea
            ...
            <div className="build__data-block">
                <p className="build__label">{ __( "Тип рейтинга" ) }</p>
                <Select
                    options={ PARTICIPANT_TYPE_OPTIONS }
                    changeValue={ getTournamentEditor( "participantType" ) }
                    defaultText={ PARTICIPANT_TYPE_OPTIONS.find(
                        option => option.value === newTournament.participantType )?.text || "Выберите тип" }
                    defaultValue={ newTournament.participantType }
                />
            </div>
            <div className="build__data-block">
                <p className="build__label">{ __( "Регистрация" ) }</p>
                <label className={ newTournament.isRegisterOn ? "admin__checkbox active" : "admin__checkbox" }
                       onClick={ e => changeTournamentField( "isRegisterOn", !(newTournament.isRegisterOn) ) }>
                    <span className="admin__checkbox-rect"></span>
                    <span className="admin__checkbox-label">{ __(
                        newTournament.isRegisterOn ? "Выключить регистрацию" : "Включить регистрацию" ) }</span>
                </label>
            </div>
            { messageOptions.text &&
