...
import {IMessageOptions, IQuestion} from "../../StoreTypes";
...
const AdminFaqManage = () => {
    ...
    const deleteHandler = async (itemToDelete: Record<string, any>) => {
        try {
            const {message}: {message: string, item: Record<string, any>} = await request(`/api/question/delete`, 'POST', {id: itemToDelete.id}, {
    ...
    const createOrSaveHandler = async (event: any, itemToSave: Record<string, any>) => {
        ...
        clearError()
        try {
            const {message, item}: {message: string, item: Record<string, any>} = await request(`/api/question/save-create`, 'POST', {...itemToSave, id: itemToSave.id ? itemToSave.id : ''}, {
                Authorization: `Bearer ${auth.token}`
            }, true)
            setMessageOptions({
    ...
    return (
        <div className="mb24">
            ...
            <form className="build__data-block" action="null" method="POST" onSubmit={(event: any) => createOrSaveHandler(event, newItem)}>
                ...
                >
                    {fields.map((field) => (<label htmlFor={field.name} className="input" style={field.type === 'textarea' ? {height: 'auto', minHeight: '64px'} : {}}>
                    ...
                    {items.map((item) => (
                        <li className="panels__item panel mb24">
                            ...
                            <div className="panel__content">
                                ...
                                <button className="panel__delete" onClick={() => deleteHandler(item)}>{__('Удалить')}</button>
                            </div>
                        </li>
                    ))}
                </ul>
