...
const TeamRegisterPopup = ( { tournament, isRegisterFormActive, setIsRegisterFormActive, refetchHandler }: {
    ...
    const fetchCurrentParticipant = async () => {
        const { participant, participantUsers } = await request(
            `/api/tournament/get-own-participant?tournamentId=${ tournament?.id }&userId=${ user?.id }`, "GET" );
    ...
    useEffect( () => {
        setDisplayTeams(
            teams.filter( ( team: ITeam ) => (team.name?.toLowerCase().includes( teamsSearchString.toLowerCase() )) ) );
        ...
        >
            ...
            <div className="popup__progressbar mb12">
                <div className="popup__progressbar-value"
                     style={ { width: `${ (currentStep / 5 * 100).toFixed( 2 ) }%` } }></div>
            </div>
            { currentStep === 0 &&
                <>
                    { !!teams.length && <div className="dropdown mb24">
                        ...
                        >
                            { newTeam.id && <div className="profileCard">
                                <img src={ getFile( newTeam.avatar ) || DefaultUserPic }
                                     alt={ newTeam.name || "Team avatar" }
                                     className="profileCard__avatar"/>
                                <div className="profileCard__top">
                                    <span className="profileCard__nickname">{ newTeam.name }</span>
                                </div>
                            </div> }
                            { !newTeam.id && <input type="text" placeholder={ __( "Выберите команду" ) }
                                                    onChange={ ( e: React.ChangeEvent<HTMLInputElement> ) => {
                                                        setTeamsSearchString( e.target.value );
                                                    } }/> }
                        </label>
                ...
                <>
                    <div className="popup__newTeam mb24">
                        ...
                        >
                            <svg width="20" height="20" viewBox="0 0 20 20" fill="none"
                                 xmlns="http://www.w3.org/2000/svg">
                                <path
                                    d="M14.1667 1.66666L17.5 4.99999M17.5 4.99999L14.1667 8.33332M17.5 4.99999H5.83333C4.94928 4.99999 4.10143 5.35118 3.47631 5.9763C2.85119 6.60142 2.5 7.44927 2.5 8.33332V9.16666M5.83333 18.3333L2.5 15M2.5 15L5.83333 11.6667M2.5 15H14.1667C15.0507 15 15.8986 14.6488 16.5237 14.0237C17.1488 13.3986 17.5 12.5507 17.5 11.6667V10.8333"
                                    stroke="white" strokeOpacity="0.75" strokeWidth="1.5" strokeLinecap="round"
                                    strokeLinejoin="round"/>
                            </svg>
                        </button>
                    ...
                    { !!newTeam.players.length && <>
                            ...
                            { newTeam.players.map( ( player: IUser, index: number ) => (
                                <li className="popup__player" key={ index }>
                                    ...
                                    { ((player.id !== user?.id) || isUserAdmin( user )) && <button
                                        className={ isUserIdIncludedInRequest( player.id ) ? "checkbox active" :
                                            "checkbox" }
                                        onClick={ () => {
                                    ...
                                    >
                                        <svg width="16" height="16" viewBox="0 0 16 16" fill="none"
                                             xmlns="http://www.w3.org/2000/svg">
                                            <path d="M13.3337 4L6.00033 11.3333L2.66699 8" stroke="white"
                                                  strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                                        </svg>
                                    </button> }
                                </li>
                        ...
                        { tournament && !tournament.participationPrice && !isLoading && <button
                            ...
                            onClick={ async () => {
                                setIsLoading( true );
                                const response = await TournamentRegisterSubmit( newTeamUsed,
                                    tournamentRegistrationUsed, clearError, token, request, setMessageOptions,
                                    refetchHandler );
                                setIsLoading( false );
                                if ( response.isOk ) {
                ...
                <>
                    ...
                    <div className="popup__payMethod pb24">
                        <ul className="popup__payMethod-items">
                            <button
                                className={ registerRequest.payMethod === "paypal" ? "popup__payMethod-item active" :
                                    "popup__payMethod-item" }
                                onClick={ () => {
                                    changeRegisterForm( "payMethod", "paypal" );
                                } }
                            >
                                <svg xmlns="http://www.w3.org/2000/svg" width="1280" height="339.345"
                            ...
                            <button
                                className={ registerRequest.payMethod === "enot" ? "popup__payMethod-item active" :
                                    "popup__payMethod-item" }
                                onClick={ () => {
                    ...
                    { registerRequest.payMethod === "paypal" &&
                        <p className="text">{ PayPalData.getText( tournament.participationPrice, false ) }</p> }
                    { !isLoading && <button
                        className="button-both-accent popup__accentButton"
                        onClick={ async () => {
                            setIsLoading( true );
                            const response = await TournamentRegisterSubmit( newTeamUsed, tournamentRegistrationUsed,
                                clearError, token, request, setMessageOptions, refetchHandler );
                            setIsLoading( false );
                            if ( response.isOk ) {
                                if ( tournament && tournament.participationPrice && response.url ) {
                                    window.location.replace( response.url );
