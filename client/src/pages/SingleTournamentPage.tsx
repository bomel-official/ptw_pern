...
import TournamentCompetitionTable from "../components/tournament/TournamentRating/tournament-competition-table";
...
import { useGetOneTournament } from "../hooks/tournament/get-one";
...
export const SingleTournamentPage = () => {
    ...
    const { data: tournament } = useGetOneTournament( slug );
    ...
    const fetchCurrentParticipant = async () => {
        const { participant: newParticipant } = await request(
                ...
                <div className="side">
                    ...
                    <div className="side__content">
                                ...
                                <div className="side__tab-headings ds">
                                    { Object.values( TournamentTabs ).map( ( tab, index ) => (
                                        <NavLink
                                            ...
                                            key={ index }
                                            className={ currentTab === tab.slug ? "side__tab-heading active" :
                                                "side__tab-heading" }
                                        >
                                            { __( `${ tab.text }` ) }
                                        </NavLink>
                        ...
                        <div className="side__content-bottom">
                            <div className="side__container">
                                <NavDropdown currentTab={ currentTab } tabs={ TournamentTabs } defaultTab={ "about" }
                                             url={ `/tournament/${ slug }` }/>
                                { !!tournament.participationPrice && !!participant && !participant.isPaid &&
                                    <div className="pb32">
                                        <div className="tournament__content">
                                            { participant.payMethod === "enot" &&
                                                <>
                                                    <p className="text">{ __(
                                                        "Вам необходимо оплатить участие на турнир." ) }</p>
                                                    <div className="flex">
                                            ...
                                            { participant.payMethod === "paypal" &&
                                                <>
                                                    <p className="text">{ PayPalData.getText(
                                                        tournament.participationPrice ) }</p>
                                                    <div className="flex">
                                                        <NavLink
                                    ...
                                    <div className="tournament__content">
                                        <div className="tournament__fund">
                                            <div className="tournament__fund-top both-borders">
                                                <h2 className="tournament__fund-heading">{ __(
                                                    "Свободные слоты" ) }</h2>
                                                <div className="tournament__fund-number">
                                                    ...
                                                    <span>{ (slots >= 0 ? slots : 0) } { __(
                                                        getSlotWord( slots >= 0 ? slots : 0 ) ) }</span>
                                                </div>
                                            </div>
                                        </div>
                                        <div className="tournament__block">
                                            <h2 className="tournament__block-subheading">{ __(
                                                "Участники турнира" ) }</h2>
                                        </div>
                                ...
                                { (currentTab === "rating") && <div className="pb104">
                                    <div className="tournament__content">
                                        <div className="tournament__block">
                                            <h2 className="tournament__block-subheading">{ __(
                                                "Результаты турнира" ) }</h2>
                                        </div>
                                        { tournament && tournament.participantType === "table" ?
                                            <TournamentCompetitionTable tournament={ tournament }/> :
                                ...
                                { (currentTab === "rules") && <div className="tournament pb104">
                                    <div className="tournament__content">
                                        <div className="tournament__block">
                                            <h2 className="tournament__block-subheading">{ __(
                                                "Правила турнира" ) }</h2>
                                            <p className="tournament__block-text">{ parseUrls(
                                                _f( tournament, "descRules" ) ) }</p>
                                        </div>
                                    </div>
                                </div> }
                            </div>
